...
public class SubCommandGroupAddPerm implements SubCommand {
    ...
    public SubCommandGroupAddPerm(CommandSender sender, String[] args) {
        this.sender = sender;
        this.args = args;
        runSubCommand();
    }
    ...
    public void runSubCommand() {
        ...
        if(args.length < 3) {
            sender.sendMessage(new ComponentBuilder(
                    prefix + ChatColor.RED + "Please use " + cc + "/group addperm <group> <perm>"
            ).create());
            return;
        }
        ...
        if(Core.permission.getGroupWrapper().hasGroupPermission(group, permName).get()) {
            sender.sendMessage(new ComponentBuilder(
                    prefix + ChatColor.RED + "This group already owns the selected permission."
            ).create());
            return;
        }

        if(permName.equalsIgnoreCase(Core.permission.getPermissionWrapper().getPERM_BYPASS())) {
            sender.sendMessage(new ComponentBuilder(
